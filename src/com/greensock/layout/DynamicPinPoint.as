 ...
 */	 
	public class DynamicPinPoint extends PinPoint {
		 ...
		 */
		public function DynamicPinPoint(target:DisplayObject, getter:Function) {
			super(getter().x, getter().y, target);
		...
		/** @private **/
		override protected function init(target:DisplayObject):void {
			_data = new LiquidData(this, target, 1, false, 0, null);
			LiquidData.addCacheData(_data.liquidStage, _data);
		}
		
		/** @inheritDoc **/
		override public function clone():Point {
			var p:DynamicPinPoint = new DynamicPinPoint(_data.target, _getter);
			p.x = this.x;
			p.y = this.y;
			return p;
		}
